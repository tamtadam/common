...
sub store {
    ...
    my $sth    = $dbh -> prepare("UPDATE sessions SET expire = TIMESTAMPADD(MINUTE,300,NOW()) where id=?") ;
    $sth->execute( $sid ) or die "ERROR\n";       
    $sth    = $dbh -> prepare("UPDATE sessions SET pid =? where id=?") ;
    $sth->execute( $pid, $sid ) or die "ERROR\n";  
    return 1;
}
